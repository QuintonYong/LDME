...
public class Ldme {
    ...
    void DivideNew(int k_bins) {
        ...
        for (int A = 0; A < n; A++) {
            ...
            //System.out.println(A + "start rot");
            // rotation
	...
	void merge(int number_groups, int tt, OnePermHashSig[] group_prop_0, int[] group_prop_1){
		...
		for (int i = 0 ; i < number_groups ; i++){
			...
			while(hm.size()>1){
                    ...
                    int A = rand.nextInt(initial_size);  //int B = rand.nextInt(initial_size);
					if(hm.get(A) == null)
                        continue;
    ...
    void merge_lsh(int number_groups, int tt, OnePermHashSig[] group_prop_0, int[] group_prop_1, int k_bins) {
        ...
        for (int j = 0; j < k_bins; j++) {
            if (random.nextBoolean()) { rot_direction[j] = 1; }
            else { rot_direction[j] = 1; }
        }
		...
		for (int i = 0 ; i < number_groups ; i++){
            ...
            OnePermHashSig[] Q_buckets = new OnePermHashSig[Q.length];
            
            for (int A = 0; A < Q.length; A++) {
                Q_buckets[A] = new OnePermHashSig(k_bins);
            }
			...
			for (int x = 0; x < Q.length; x++) {
				...
				for(Integer node : hm_x.keySet()) {
                    int permuted_h = h[node];
                    int permuted_bin = permuted_h / bin_size;

                    if (Q_buckets[x].sig[permuted_bin] == -1 || permuted_h % bin_size < Q_buckets[x].sig[permuted_bin]) {
                        Q_buckets[x].sig[permuted_bin] = permuted_h % bin_size;
                    }
                }
                ...
                for (int A_bin = 0; A_bin < k_bins; A_bin++) {
                    ...
                    if (Q_buckets[x].sig[A_bin] == -1) {
                        int j = (A_bin + direction) % k_bins;
                        if (j < 0) { j += k_bins; }
    
                        int c = 0;
                        while (Q_buckets[x].sig[j] == -1 && c < k_bins) { 
                            j = (j + direction) % k_bins;
                            if (j < 0) { j += k_bins; }
    
                            c++;
                        }
                        Q_buckets[x].sig[A_bin] = Q_buckets[x].sig[j];
                    }               
                }
			...
			while(hm.size()>1){
                ...
                while (true) {
                    int j = rand.nextInt(initial_size);
                    if (hm.get(j) != null && j != A) {
                        idx = j;
                        break;
                    } 
                }
				...
				/*for (int j = 0 ; j < initial_size ; j++) {
					...
					if (j==A) continue;
					if (!(Q_buckets[A].equals(Q_buckets[j]))) continue;
...
//====================== Finds the nodes in each supernode with index key
	...
	void test(int Iter) {
        ...
        //int k_bins = 3; // cnr-2000 325557
        //int k_bins = 4; // eu-2005 862664
        //int k_bins = 9; // enron 69244
        //int k_bins = 5; // hollywood-2009
		...
		for (int iter = 1 ; iter <=Iter ; iter++){
			...
			if (use_lsh_merge == 1) { merge_lsh(cnt_groups, iter, group_prop_0, group_prop_1, k_bins_merge); }
            else 			   		{ merge(cnt_groups, iter, group_prop_0, group_prop_1); }
